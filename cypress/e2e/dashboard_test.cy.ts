describe('Dashboard', () => {
  ...
  it('should navigate to the dashboard', () => {
    cy.get('[data-cy="pathways-title"]').contains('My Pathways')
  });
  ...
  it('should render expected pathways', () => {
    ...
    cy.get('[data-cy="pathway-card"]').then((items) => {
      ...
      expect(items[0]).to.contain.text('View Pathway');
      ...
      expect(items[1]).to.contain.text('View Pathway');
      expect(items[0].children[0].children[1]).to.contain.text('70%');
      expect(items[1].children[0].children[1]).to.contain.text('70%');
    })
  });
});
