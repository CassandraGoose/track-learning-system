...
test('navigates to the dashboard from the get started link', async ({ page }) => {
  ...
  await page.waitForURL('http://localhost:3000/pathways');
  await expect(page).toHaveURL('http://localhost:3000/pathways');
});
...
test('renders the video', async ({ page }) => {
  await expect(page.getByTestId('splash-video')).toBeVisible();
});
